...
//Actions for owners
import { getAllOwners, validateData, getDataOwner, verifyCreationOwner } from '../app/reducer/propietarioSlice';

//Actions for data
import { getAllDataAccountType, getAllDataBank, createAccountBank, selectAccountBank, getAllProductsType } from '../app/reducer/getDataSlice';

//Direcctions
import { getAllDepartments, getAllMunicipies, createDirection } from '../app/reducer/directionSlice';
...
export const search_all_data_owner = (params) => {
    return async(dispatch) => {
        ...
        try{
            ...
            const token = '31231dda7ujsjshdhshjdshdjhsd';
            const { data } = await request(http.get, '/api/propietario/searchOwner', { token, data: params });

            console.log(data);

            dispatch(getDataOwner(data));
        }
        catch(err) {
            console.error(err);
        }
...
//Seleccionar todos tipos de producto para el almacen
