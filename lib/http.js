...
//Actions for owners
import { getAllOwners, validateData, verifyCreationOwner, updateOwner, uploadImgs, deleteOwner, dataOwnerEcom } from '../app/reducer/propietarioSlice';
...
//Store 
import { createStore, updateStore, getDataStore, getAllDataStores, deleteStore } from '../app/reducer/storeSlice';
...
export const search_propietario = (params) => {
    return async (dispatch) => {
        try {
            //debugger;
            const token = '31231dda7ujsjshdhshjdshdjhsd';
            const { data } = await request(http.post, '/api/propietario/searchPropietarios', { token, data: params });
            dispatch(getAllOwners(data));
...
//Buscar almacen
export const store_search  = (params) => {
    return async (dispatch) => {
        try{

            const token = '31231dda7ujsjshdhshjdshdjhsd';
            const { data } = await request(http.post, '/api/store/searchStore', { token, data: params });
            dispatch(getAllDataStores(data));
...
//Delete a store
export const delete_store = (params) => {
    return async(dispatch) => {
        try{

            const token = '31231dda7ujsjshdhshjdshdjhsd';
            await request(http.post, '/api/store/deleteStore', { token, data: params});
            dispatch(deleteStore([1]));
...
//update data ecomtrading
export const get_all_data_owner_ecom = (params) => {
    return async(dispatch) => {
        try{
            const token = '31231dda7ujsjshdhshjdshdjhsd';
            const { data } = await request(http.post, '/api/propietario/updateOwner', { token, data: params});
            dispatch(dataOwnerEcom(data));
        }
        catch(err){
